...
MATH = {          # inline mathematics: value is the body of $...$
    ...
    'K(x) − K(y)': 'K(x) - K(y)',
    'K^t': 'K^t',
    'K^t(x)': 'K^t(x)',                       # B50 Phase 3 (Referee E (e1)): K^t defined in section 2.1
    'K^t(x | y)': 'K^t(x \\mid y)',
    'k_0': 'k_0',
    'kT': 'kT',
    'kT ln 2': 'kT \\ln 2',
...
\text{open: does no io-OWF force BEM?} & barrier on record: Proposition 10 (conditional); Proposition 9 is the barrier for the top of the skewed sandwich
\end{array}
\]""",
# --- section 1 ---
...
r"""\[
\begin{aligned}
&\text{some polynomial-time samplable process has, at every polynomial time bound,} \\
&\qquad\text{mean production above } c \log n \text{ for infinitely many } n\text{, for every } c \\
...
\usepackage[bottom]{footmisc}  %% footnotes at the foot of the page even when the page ends early (owner, 20 Sep 2026)
...
%% B50 Phase 3 (20 Sep 2026): the Phase 2 form read \pagegoal-\pagetotal, which is stale when the paragraph
%% before the lead-in straddles a page (it broke page 5 after one line, 59 -> 60 pp.); this is the glue-and-
%% penalty idiom of the needspace package, decided by the page builder itself.
\newcommand{\keepwithtable}{\par\begingroup\dimen0=7\baselineskip
  \vskip 0pt plus \dimen0\relax \penalty-100 \vskip 0pt plus -\dimen0\relax
  \vskip\dimen0 \penalty9999 \vskip-\dimen0 \vskip 0pt\relax\endgroup}
...
CONT = re.compile(r'^(\((?:a|b|c|i|ii|iii|1|2)\)|\*In words|where c_0|and the same for|and likewise|'
...
def main():
    ...
    while k < len(bl):
        ...
        if kind == 'fig':
            ...
            # markdown caption reads "*Figure N. <caption>.* *In words: ...*"; LaTeX supplies the
            ...
            m = re.match(r'^\*Figure (\d+)\. (.+?)\* (\*In words: .+\*)$', nb[1]) if nb and nb[0] == 'p' else None
            if not m:
                raise ValueError('tex-figure block not followed by an italic "*Figure N. ...* *In words: ...*" caption')
            n_figs += 1
        ...
        if title is not None and author is None:
            ...
            # title block (owner, 20 Sep 2026, B50 Phase 3, as in ECCC TR26-199): the e-mail is a first-page
            # footnote - a comma-separated part containing "@" is wrapped in \thanks{} with no separator
            # before it; any other parts (name; an affiliation if the header carries one) are joined with \\.
            # The date is the plain date: the header's leading "Version of " is not emitted.
            ...
            for part in parts:
                if '@' in part:
                    ...
                    author += (' \\\\ ' if author else '') + part
            assert author.count('\\thanks{') == 1, 'header line: expected exactly one e-mail part: %r' % parts
            if date.startswith('Version of '):
                date = date[len('Version of '):]
            k += 1
            continue
