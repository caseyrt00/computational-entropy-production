...
- **Theorem 1.** The exponential average of minus the computational entropy production lies between two fixed polynomials in n, and the mean is at least −O(log n), for every polynomial-time samplable process, unconditionally. The proof is two Kraft sums, one explicit pair and Jensen's inequality.
...
- **Theorem 3.** For the difference of the two one-sided productions of a pair, the Zurek defect, the existence of one-way functions forces both exponential moments to be superpolynomial infinitely often; whether the converse holds is the paper's open problem.
- **Theorem 4.** A process has small computational entropy production on a pair exactly when a fast machine, handed the output, can guess the input with about the right odds, and the universal sampler always serves as the witness. It is assembled from known parts, HILNO's universal sampler and the coding theorem with an auxiliary input, and its content is the identification: dissipation in this paper's sense is the failure of the reverse conditional to be dominated by an efficient sampler (Section 6).
...
*Plain reading: a fast process that dissipates, in the sense defined in Section 2, is the same thing as a one-way function, and a one-way function forces P ≠ NP. The equivalence is Theorem 2; the last arrow is standard (Section 2.3) and is not known to reverse. Theorem 1 says the left-hand quantity cannot be very negative for any process; Theorem 3 refines the left-hand side by a second, antisymmetric quantity and leaves one direction open.*
...
The sandwich around the open problem, Sections 5 and 8, drawn:
...
Where the paper is explicit about its own limits: Section 1.1 (what the results are not), the Conventions below (§1.3, labels and read-status), and Section 9.5 (what is attributed to whom).

### 1.3 Conventions

**Labels.** Every theorem, lemma, proposition and corollary carries one of: **Proved** (a written proof appears here, or an exact pointer to a read source is given); **Proved-conditional** (proved from a named hypothesis or a named source result, the dependence stated); or **informal** (Proposition 11 alone: a reading of the sources, offered without a proof). Claims in the prose that are expected but not proved say so in words; one paragraph of §8.1 is a speculation and says so.

**Read-status.** The reference list states, for every work cited, whether it was read (with the parts read named) or read only through another source that was read ("cited via"). Citations in the body name the work and the place cited, and nothing else.
...
**Numbering.** Theorems 1, 2, 3 and 4 are the paper's four theorems and are numbered as such throughout. Theorem 0 is a known theorem restated for comparison. Definitions, lemmas, propositions, corollaries and remarks are each numbered 1, 2, 3, … in order of appearance across the whole paper.

**Direction of argument.** Physics is the frame of this paper, not its subject. Every statement about a physical system is a parallel, named as such. No sentence of this paper runs from a physical fact to a conclusion about complexity. In particular "efficiently samplable" is a condition on the running time of a sampler and is never presented as a physical condition.
...
*Plain reading: the fewest bits k such that, for at least two thirds of the random strings r, some k-bit program prints x within time t when it is also handed r. It is time-bounded description length when everyone shares a random string.*
...
*Plain reading: if a one-way function exists, no fast randomized algorithm solves NP, and so P ≠ NP. The arrows are not known to reverse: P ≠ NP may hold with NP hard only in the worst case (Heuristica) or hard on average but with no one-way function (Pessiland), and which of the five worlds holds is open.* The first arrow is the observation that if NP ⊆ BPP then every polynomial-time function is invertible in probabilistic polynomial time by search-to-decision; the second is BPP ⊇ P. Nothing in this paper touches the converse of either arrow.
...
*Plain reading: how many more bits the bounded observer's reverse description of the pair needs than the sampler's own probabilities say it should. The first bracket is the excess for recovering x from y; the second is the excess for describing y. Positive means the observer cannot reverse the pair as well as the process produced it.*
...
**Proved.** On supp D, D(x, y) = 2^{σ^rev_D(x,y)} m_R(x, y) by Definition 5; sum over the pairs with σ^rev_D = s. ∎
...
*Plain reading: the exponential average of minus the entropy production is pinned between two fixed polynomials in n. It is the total reverse weight of the sampler's support, which is at most about n squared and at least one over a polynomial.*
...
**Proved.** The proof uses seven lemmas, stated in §3.2; the proof itself is §3.3.
...
**Proved.** Consider the sampler of [Lu, Oliveira and Zimand 2022, Lemma 36], written LOZ below, at fixed n: pick j ∈ {1, …, n + d} uniformly, pick w ~ {0,1}^{t(n)} and M ~ {0,1}^j uniformly, run U(M, w) for t(n) steps, output the result. By Definition 1 it outputs x with probability at least (1/(n + d)) · (2/3) · 2^{−pK^t(x)}. Its output probabilities sum to at most 1. ∎
...
**Proved.** The same sampler with y on the extra tape; this is the universal sampler USamp of [HILNO, Definition 21 and Proposition 22]. ∎

**Lemma 4 (monotonicity in t).** If t ≤ t′ then pK^{t′}(x) ≤ pK^t(x) and pK^{t′}(x | y) ≤ pK^t(x | y).

**Proved.** A program halting within t steps reads at most t bits of the random tape, and the first t bits of a uniform r′ ∈ {0,1}^{t′} are uniform. ∎
...
**Proved.** E_{x ~ D(· | y)} [ 2^{−pK^t(x | y)} / D(x | y) ] = Σ_x 2^{−pK^t(x | y)} ≤ (3/2)(n + d) by Lemma 3. Markov's inequality gives Pr[ 2^{−pK^t(x|y)} / D(x|y) ≥ 2^{α} (3/2)(n + d) ] ≤ 2^{−α}, which is the statement. ∎
...
**Proved.** The first line is Lemma 5 for D_2; the second is Lemma 7 in its marginal form. ∎
...
**Proved** as an equivalence, by HILNO. Whether either side holds is open.
...
**Proved.** Both directions from HILNO's ingredients, written out below. **Not new:** Theorem 2 is [HILNO Theorem 1, (1)⇔(3)] packaged in expectation form, (1) ⇒ (2) as stated and HILNO's Lemma 20 argument for the converse; its value is the sentence it licenses, *one-way functions exist if and only if some efficiently samplable process dissipates.*
...
**Proved** (this is HILNO's Lemma 20 argument). Suppose not: there are c, q, t such that for all large n, Pr_z[good(z)] ≥ 1/q(n), where good(z) means pK^t(z | f(z)) ≤ log |f^{−1}(f(z))| + c log n. Call an image y good if at least a 1/(2q) fraction of its preimages are good. Since Pr_z[good(z)] = Σ_y D_2(y) · (fraction of good preimages of y), and that fraction is at most 1 on good images and less than 1/(2q) on the rest, Pr_z[f(z) is a good image] ≥ 1/(2q). Fix a good image y and set k = ⌈log |f^{−1}(y)|⌉ + c log n. By [HILNO Proposition 22], the universal sampler USamp(1^n, 1^t, y) outputs each fixed z with pK^t(z | y) ≤ k with probability Ω(1 / (n 2^k)) = Ω(1 / (n^{c+1} |f^{−1}(y)|)); summing over the at least |f^{−1}(y)| / (2q) good preimages of y, USamp outputs *some* preimage of y with probability Ω(1 / (q n^{c+1})). The algorithm "on input y, run USamp(1^n, 1^t, y) and output the result" therefore inverts f with probability at least (1/(2q)) · Ω(1 / (q n^{c+1})) = 1 / poly(n) for all large n. That contradicts Definition 3, which requires every polynomial-time inverter to succeed with probability below 1/q′(n) for infinitely many n, for every polynomial q′. ∎
...
*Plain reading: how many more bits the bounded observer pays to describe the pair as "y first, then x from y" than as "x first, then y from x", both within the same time t. Positive means the reverse-order description is the dearer one.*
...
*Plain reading: first line, the defect is "what x tells the observer about y" minus "what y tells the observer about x", at time t; it vanishes exactly when time-bounded mutual information is symmetric. Second line, the defect is the time-t cost of going from y to x rather than from x to y, minus the value symmetry of information (Theorem 0) would assign to that difference, namely the difference of the two individual costs. This is the sense in which σ_t is Zurek's cost at a time bound minus its unbounded value.*

**Proved.** (i) By Definition 4 both weights are functions of x, y, t and U. On supp D, σ^rev_D − σ^fwd_D = log(D / m_R) − log(D / m_F) = log m_F − log m_R. (ii) m_F(y, x) = 2^{−pK^t(y) − pK^t(x | y)} = m_R(x, y), and likewise m_R(y, x) = m_F(x, y). (iii) Rearrangement of the four terms of Definition 6. ∎
...
*Plain reading: bounded exponential moments sit between "NP is easy" and "no one-way functions". Whether either arrow reverses is not decided here; whether the second one does is the paper's open problem, Section 8.*

**Proved.** Parts (a) and (c) from the lemmas of Sections 3 and 5; part (b) from [HILNO, Theorem 4, p. 9].
...
*Plain reading: the second half of σ is small for every process, by Lemma 5. The first half is small because the backward program is a real fast program, and a real fast program that lands on x with probability δ hands x a fast description of about log 1/δ bits, which is the coding theorem, used here with the end state as a free extra input. That the coding theorem allows the extra input is Lemma 6.*
...
**Proved**, the contrapositive of Proposition 2.

*Plain reading: a process shows a computational arrow only if no fast program can run it backward with even a fixed fraction of the right odds. Where the backward run is easy, the arrow is gone.*
...
**Proved.** Suppose instead that some S dominates within n^c on a set Y_n with D_2(Y_n) ≥ 1 − 1/n for all large n. By (⇐), σ^rev_D ≤ c′ log n on every pair with y ∈ Y_n, at every t ≥ p′(n) := t_D + p(t_S(n)). On the remaining pairs, of D-mass at most 1/n, σ^rev_D ≤ 2(n + d) pointwise, since each of pK^t(x | y) and pK^t(y) is at most n + d and log D(x, y) ≤ 0. So E_D[σ^rev_D] ≤ c′ log n + 2(n + d)/n ≤ (c′ + 1) log n for all large n at t = p′(n), and D does not dissipate: Theorem 2's condition fails at p = p′, C = c′ + 1. ∎

*Plain reading: a process with a computational arrow is one that no fast machine can run backward with the right odds, even on most ends. Corollary 1 said "not exactly, on every end"; this says "not even approximately, on all but a 1/n slice of ends", for infinitely many lengths.*
...
**Proved.** As Lemma 11: amplify both programs to success probability 0.9 by GKLO22's Lemma 21, run the first on z with its block of the random tape and keep its output y on the work tape, run the second on that y with a disjoint block, an O(log t)-bit delimiter separating the blocks; the success probability is at least 0.81 ≥ 2/3 and the time O(t) + O(n). ∎
...
*Plain reading: running two processes in a row cannot dissipate more, on average, than the two dissipate separately, plus the information in the middle state that the two ends together do not determine, plus about log t bits. When the first process is deterministic, or whenever the two ends fix the middle, that extra term is zero.*
...
**Proved.** (a) Write σ_{GF}, σ_F and σ_G at the times stated and Δ := σ_{GF} − σ_F − σ_G. By Definition 5,
...
*Plain reading: a function that no fast algorithm inverts with probability better than two to the minus s dissipates at least s bits per use, minus the bits its output loses outright and minus about log n. For a permutation nothing is lost, and s bits of security are s bits of dissipation, at any clock below the security time and within about log n bits.*
...
*Plain reading: whatever a fast machine can do to guess the start from the end, running a random short program on the end does within a polynomial factor, at a polynomially larger clock. It is the time-bounded analogue of the universality of the universal semimeasure.*

**Proved; known in substance**, being the content of Lu, Oliveira and Zimand's optimal coding theorem together with HILNO's Proposition 22: Lemma 6 gives pK^{p(t_S)}(x | y) ≤ log 1/S(x | y) + O(log t_S), and Definition 8's bound gives USamp_{p(t_S)}(x | y) ≥ 2^{−pK^{p(t_S)}(x | y)}/(a n). ∎ It is the universal-witness clause of Theorem 4 with the process D removed.
...
**Open Problem (status: open).** Does the non-existence of infinitely-often one-way functions force bounded exponential moments of the same-time Zurek defect for every polynomial-time samplable pair distribution? That is, does
...
*Plain reading: the same comparison of the two orders of description as σ, except that the order being bounded above is given polynomially more time. Two objects, one per direction; Σ⁻(x, y) = Σ⁺(y, x); they are not antisymmetric.*
...
The first arrow of the left chain is standard and relativizes (an errorless heuristic for a problem that is easy in the worst case is the worst-case algorithm itself). The open problem asks whether the last arrow of the left chain reverses for the same-time object, that is, whether "no io-OWF" reaches all the way back to BEM.

### 8.2 Remark: the time-skewed variant

**Proposition 7 (the skewed sandwich).** **Proved** from [GKLO22, Lemma 26(1) together with the sentence following its two items, which states the special case without a conditioning string, p. 16:22] and Lemma 11.
...
**Proof.** (a) GKLO22's Lemma 26(1), in the special case without a conditioning string that the source states in the sentence following the lemma, gives: if DistNP ⊆ AvgBPP there are polynomials p, p_0 such that for all large n, all t ≥ p_0(n) and every x, y ∈ {0,1}^n,
...
**Proposition 8 (the top of the skewed sandwich, lowered).** **Proved-conditional** on [GKLO22, Lemma 26(1)] as read, specifically on the fact that its proof uses DistNP ⊆ AvgBPP only once, to obtain the Lemma-6 algorithm for (L, D) [GKLO22, Lemma 6 and the proof of Lemma 26(1), pp. 16:22–16:24].
...
**Proposition 9 (no relativizing argument collapses the skewed sandwich to NP ⊆ BPP).** **Proved-conditional** on (i) the oracle of [Hirahara and Nanashima 2021, Theorem 3 (ECCC TR21-161, §6)]: an oracle O with
...
and (ii) GKLO22's remark after their Definition 17 that all the results of their Section 3 also hold with any oracle [GKLO22, §3, p. 16:18], which covers Lemma 26 (their §3.4).
...
**Proposition 10 (the bottom arrow does not reverse by any relativizing argument).** **Proved-conditional** on (i)–(iii); hypothesis (iii) is itself inferred from the construction as read, not proved. (i) The construction of O_a and Fact M as read [Hirahara and Nanashima 2022, §4]. (ii) Theorem 15 as read [Hirahara and Nanashima 2022, §5]. (iii) HILNO's Lemma 33 and Proposition 38 [HILNO, Lemma 33, §5.1, p. 37, and Proposition 38, §5.2, p. 42], taken to relativize; the reason for the inference: Lemma 33 is the search-to-decision reduction of Ben-David, Chor, Goldreich and Luby [1992] with pairwise-independent hashing, giving conditional extrapolation from DistNP ⊆ HeurBPP, and Proposition 38 runs the extrapolator on f(x) to invert f; both are black-box in the oracle. Part (a) uses (i) only. Let m = 2n and let D be the sampler that draws z, w ∈ {0,1}^n uniformly and outputs (x, y) := ((z, w), R_z(w) 0^n), a pair of m-bit strings, with n oracle queries; at odd lengths it outputs (0^m, 0^m). Then:
...
*Plain reading: the oracle world that breaks the same-time bound also breaks the skewed bound, because giving the reverse-order description polynomially more time still leaves it inside the masked levels, where the rows are hidden. In that world the one counting problem is hard too, so the two sides of the open converse are both false there, and both true in the two errorless worlds. No known oracle world tells them apart; what stands in the way of the converse is a matter of size, not a black-box impossibility.*
...
Conversely, for any set of mass at least 1 − 2/q that carries only polynomial moment, Theorem 3(a) says that if io-OWFs exist the weight outside it is superpolynomial infinitely often. So the open problem is exactly whether a set of polynomially small mass can carry superpolynomial exponential weight. Note the quantifier order: G and p depend on q, whereas BEM asks for one p and every tail level. The reading from physics is Jarzynski's: exponential averages of work are dominated by rare realizations, and the trajectories that carry them look like typical reverse trajectories run backward [Jarzynski 2006]. Physics has no theorem forcing the rare realizations to carry bounded weight, and for σ_t there is no normalization identity to do the work (Remark 4). The reading runs from computation to physics; nothing above rests on it.
...
*Plain reading: errorless easiness gives the skewed moment bound and so no one-way functions; error-prone easiness gives no one-way functions but provably not the moment bound by black-box means, the crossed arrow meaning that no proof working in every oracle world exists. So the moment bound is a condition of the errorless kind, and one-wayness is not.*
...
*Plain reading: the moment bound sits between "description length is computable on average without lying" and "description length is computable on average, lying allowed". That is where Hirahara puts ordinary symmetry of information.*
...
*Plain reading: two different easiness conditions each force the skewed moment bound: the counting problem being easy on average without lying, and a fast test that calls most strings random given any helper. Neither is known to imply the other.*
...
*Plain reading: the bits the bounded observer's backward story needs beyond the bits the forward story needed. Near zero means the observer can rebuild the action from the end state; large means the action is gone for that observer.*
...
*Plain reading: when nothing is one-way, the observer's guess "no arrow" is right on almost every pair, and, for the plain time-bounded description length of a random string, it can even compute that length outright. When something is one-way, the observer gets that plain length wrong on a noticeable slice of random strings; whether it also fails to estimate this paper's σ is not proved here.*
...
*Plain reading: in physics, "the rules can't give you the arrow" is proved and is fine. In computing, "the rules can't give you the arrow" is only proved for a few kinds of proof, and if it were fully true the strong version of the law would be nearly false. Same question, opposite answer.*
...
The two positions sit at the two ends of one scale. At the P = NP end is the Aaronson–Kardes–Hartle demon: if NP is easy, the appearance of the law can be undone. At the one-way-function end are Theorems 1 and 2: the mean production of every fast process is bounded below unconditionally, and it is unbounded above for some fast process exactly when one-way functions exist. The distance between the two ends is the converse of Theorem 3, the open problem of Section 8: whether "no one-way functions" already forces bounded exponential moments of the defect for every fast process, or whether that condition sits strictly between "no one-way functions" and "NP is easy".
...
**Status codes.** Every row of both tables carries one of four codes. **(i)** definition on both sides: the two cells name the same kind of object and neither is a claim. **(ii)** proved on both sides: each cell is a theorem, or the computing cell is a proved equivalence between two computing statements, whose truth may itself be open. **(iii)** theorem on one side, an open condition on the other. **(iv)** shape match: the two cells have the same form and no theorem says they are the same kind of thing; the row is a modelling choice or a reading. Read-status is stated as in the Conventions. The codes describe the standing of each row's claims. The physics column is a frame throughout: no computing cell draws on it, and no row runs from a physical fact to a conclusion about complexity.
...
| 7 | integral fluctuation theorem, eq. (4): ⟨e^{−ω}⟩ = 1 | Theorem 1(a): n^{−O(1)} ≤ E_D[2^{−σ}] ≤ O(n²) | (ii), with "= 1" weakened to "pinned between two polynomials" | §3.1 |
...
This paper was written with an AI system (Claude, Anthropic) as a research collaborator: the literature audits, the computations and the first drafts of many proofs were produced in that collaboration, the typesetting and the checks that the PDF matches the source are scripted, and three blind referee passes on disjoint parts of the paper were run the same way. The judgement, the claims and the mistakes are the author's. I thank Shinji Takesue for correspondence on the reversible cellular automata that started this work.

## References

Every entry states its read-status, one of the two of the Conventions: **Read**, with the parts read named; or **cited via X**, where X is a source that was read and the original was not opened. Source files, where held, are in `problems/one-way-functions/passes/pass16/sources/`. Short names used in the text: HILNO = Hirahara, Ilango, Lu, Nanashima and Oliveira 2023; LOZ = Lu, Oliveira and Zimand 2022; GKLO22 = Goldberg, Kabanets, Lu and Oliveira 2022; KK = Kabanets and Kolokolova 2025.
...
- Aaronson, Bouland, Fefferman, Ghosh, Vazirani, Zhang, Zhou (2024). Quantum pseudoentanglement. ITCS 2024; arXiv 2211.00747. — Read (abstract, Corollary 1.0.1 and the AdS/CFT passages of §3.4, via ar5iv, pass 20, thread B30, `passes/pass20/research/b30_complexity_meters.md`; the PDF is not held in `sources/`; author initials not verified, surnames as recorded in research note B30 §0, entry S2).
...
- Bennett, C. H. (1973). Logical reversibility of computation. IBM J. Res. Dev. 17(6):525–532. — Read (abstract, p. 525, p. 526; `sources/bennett1973_logical_reversibility.pdf`; read-note `passes/pass16/research/16-reversible-computing-audit.md`).
...
- Fox, M., Karamchedu, C., Mygdalas, S. (2026). Semiclassical gravity efficiently solves NP-complete problems. arXiv 2606.14806 (v1, 11 June 2026). — Read (abstract, §I, §II with the statement of the Physical Extended Church–Turing Thesis, and §V including footnotes 9–12; §§III–IV skimmed by heading; read in pass 20, thread B23's research note `passes/pass20/research/mountain2_physics_tests.md`, which carries the read-status ledger; the PDF is not held in `sources/`).
...
- Kabanets, V., Kolokolova, A. (2025). Chain rules for time-bounded Kolmogorov complexity. ECCC TR25-089. — Read (abstract, §1, §1.3, Appendix A, bibliography; and §8, Read in pass 20, thread B21; §2.3 Definition 2.11 (p. 12), §2.4 Lemmas 2.15–2.16 and Corollary 2.19, §3.3 Theorem 3.4 with its proof (pp. 21–22), §4.2 Theorem 4.3 with its proof (p. 24), footnote 2 (p. 2) and Appendix B (Theorem B.2, Lemma B.1), read in pass 20, threads B31, B36 and B37, `passes/pass20/research/b31_instance_checkers.md`, `passes/pass20/research/b36_worst_case_hub.md`, `passes/pass20/wallA_attempt.md`; `sources/kk_eccc_tr25-089.pdf`).
...
- Liu, Y., Pass, R. (2020). On one-way functions and Kolmogorov complexity. FOCS 2020, pp. 1243–1254; ePrint 2020/423. — Read (pp. 1–5 of the ePrint version: abstract, §1, §1.1, the start of §1.2; Theorem 1.1 and the definition of mild average-case hardness; read in pass 04, `passes/pass04/liupass2020.md`; the PDF is not held in `sources/`).
...
- Lu, Z., Oliveira, I. C., Zimand, M. (2022). Optimal coding theorems in time-bounded Kolmogorov complexity. ICALP 2022; arXiv 2204.08312. — Read (§§1, 2, 5; §§3–4 skimmed; the parts used: Theorem 5 = their Theorem 30, Lemma 36; Lemma 31 and the proof of Theorem 30, §5.1, pp. 25–26, read in full in pass 20 for Lemma 6, reading note `passes/pass20/research/b24_coding_theorem_input.md`; `sources/lu_oliveira_zimand_2022_arxiv2204.08312.pdf`).
- Marletto, C., Deutsch, D., Vedral, V. (2026). Tests of constructor theory. arXiv 2606.07352 (v1, 5 June 2026). — Read (§1, §1.1, §2.3, and the work-media passage of §3; §§2.4 and 4–5 skimmed by heading; read in pass 20, `passes/pass20/research/mountain2_physics_tests.md`, which carries the read-status ledger; the PDF is not held in `sources/`).
...
- Shiraishi, N., Takesue, S. (2025). Complete ergodicity in one-dimensional reversible cellular automata. J. Stat. Phys. 192:165; arXiv 2408.06691. — Read (§1, §2, §8, §9; `sources/shiraishi_takesue_2025_arxiv2408.06691.pdf`; read-note `passes/pass16/research/13-shiraishi-takesue-2025-read.md`).
...
- Takesue, S. (1989). Ergodic properties and thermodynamic behavior of elementary reversible cellular automata. I. Basic properties. J. Stat. Phys. 56(3/4):371–402. — Read (abstract, §1, §2, §5, §6; §3–§4 by heading; `sources/takesue1989_ergodic_I.pdf`; read-note `passes/pass16/research/14-takesue-1989-read.md`).
...
- Wolfram, S. (2023). Computational foundations for the second law of thermodynamics. writings.stephenwolfram.com, February 2023. — Read (pass 04; read-note `passes/pass04/wolfram2023.md`).
